...
df1=pd.read_csv("Mistral_Final_Pred_HSA.csv")                              #Change
c=0
for index,row in df1.iterrows():
    if row['mistral_Pred']==row['label']:                          #Change
        c+=1
print("Accuracy of Data without Sarcasm Augmentation:")
print(c/df1.shape[0])

df2=pd.read_csv("mistral_Pred_sa.csv")                             #Change

df1_filtered = df1[~((df1['label'] != df1['mistral_Pred']) & (df1['Sarcasm_Col'] == 'Sarcastic'))]          #Change
